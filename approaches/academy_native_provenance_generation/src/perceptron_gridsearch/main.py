...
def start_agent_process(role: str, registration, workflow_id: str, campaign_id: str, *extra_args: str):
    env = os.environ.copy()
    env["ACADEMY_FLOWCEPT_ENABLED"] = "1"
    env["FLOWCEPT_WORKFLOW_ID"] = workflow_id
    env["FLOWCEPT_CAMPAIGN_ID"] = campaign_id
    command = [
        sys.executable,
        "run_agent.py",
        "--role",
...
async def main() -> None:
    ...
    with Flowcept(
        workflow_name="academy_perceptron_gridsearch_academy_native_provenance_generation_driver",
        workflow_subtype="academy_perceptron_gridsearch_driver",
        workflow_args={
            "approach": "academy_native_provenance_generation",
            "use_case": "perceptron_gridsearch",
        },
        check_safe_stops=False,
    ) as flowcept:
        os.environ["ACADEMY_FLOWCEPT_ENABLED"] = "1"
        os.environ["FLOWCEPT_WORKFLOW_ID"] = flowcept.current_workflow_id
        os.environ["FLOWCEPT_CAMPAIGN_ID"] = flowcept.campaign_id

        with log_context(log_config):
            logger.info("start academy perceptron gridsearch native provenance generation")
            processes = {}
            async with await Manager.from_exchange_factory(
                ...
                try:
                    processes["training_worker"] = start_agent_process(
                        "training_worker", training_registration, flowcept.current_workflow_id, flowcept.campaign_id
                    )
                    processes["evaluator"] = start_agent_process(
                        "evaluator", evaluator_registration, flowcept.current_workflow_id, flowcept.campaign_id
                    )
            ...
            logger.info("end academy perceptron gridsearch native provenance generation")
