...
def parse_args():
    parser = argparse.ArgumentParser(description="Run one Academy-native Perceptron GridSearch agent process.")
...
async def main() -> None:
    ...
    with log_context(log_config):
        logger.info(
            "starting academy-native agent process",
        ...
        logger.info(
            "stopped academy-native agent process",
            extra={"academy.agent_id": registration.agent_id, "agent_role": args.role, "pid": os.getpid()},
        )
