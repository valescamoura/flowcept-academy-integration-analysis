...
def parse_args():
    parser = argparse.ArgumentParser(description="Run one message-stream Academy Perceptron GridSearch agent process.")
...
async def main() -> None:
    ...
    with log_context(log_config):
        logger.info(
            "starting message-stream agent process",
        ...
        logger.info(
            "stopped message-stream agent process",
            extra={"academy.agent_id": registration.agent_id, "agent_role": args.role, "pid": os.getpid()},
        )
